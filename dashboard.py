...
def dump_update():
    with sqlite3.connect(app.config['DB']) as conn:
        conn.execute("CREATE TABLE IF NOT EXISTS sms (id INTEGER PRIMARY KEY, type TEXT, date TEXT, address TEXT, status TEXT, message TEXT);")
        conn.execute("CREATE TABLE IF NOT EXISTS calls (id INTEGER PRIMARY KEY, type TEXT, date TEXT, number TEXT, name TEXT, duration TEXT);")
        conn.execute("CREATE TABLE IF NOT EXISTS contacts (id INTEGER PRIMARY KEY, number TEXT, name TEXT);")
